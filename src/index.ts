...
import { getNodeByIndex, isSameListKind, replacePlaceholders, sanitizeText } from './utils';
...
async function transpileDocument(doc: DocumentNode, rules: any, config: any): Promise<string> {
  ...
  async function transpileNode(node: DocumentNode, indexArray: number[]): Promise<string> {
    ...
    try {
      const transpileNodeType = (await import(`./nodes/${nodeType}.ts`)).default;
...
async function generateLatexDocument(doc: DocumentNode, settings: Settings): Promise<string> {
